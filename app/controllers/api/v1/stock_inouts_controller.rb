class Api::V1::StockInoutsController < Api::V1::BaseController
  ...
  def index
    ...
    if category =='0'
      result = StockInout.get_in_stock_edit_data(params[:shipper_id],  warehouse_id, inout_on)
    else
      result = StockInout.get_out_stock_edit_data(params[:shipper_id], warehouse_id, inout_on)
    end
    render json: result
  end
  def inventory
    ...
    request_stock_in_params.each do |record|
      ActiveRecord::Base.transaction do
        stock_in_params = ActionController::Parameters.new(record).permit(
          :stock_inout_id, :warehouse_id, :shipper_id, :product_id, :category, :amount, :inout_on,
          :handling_fee_rate, :storage_fee_rate, :lot_number, :weight, :warehouse_category_id
        )
        stock = Stock.where(
        ...
        if stock_in_params[:stock_inout_id] == 0
          StockInout.create(
            stock_id:             stock_id,
            category:             0,
            ...
            warehouse_category_id:stock_in_params[:warehouse_category_id]
          )
        else 
          StockInout.find(stock_in_params[:stock_inout_id]).update(
            stock_id:             stock_id,
            inout_on:             stock_in_params[:inout_on],
            amount:               stock_in_params[:amount],
            handling_fee_rate:    stock_in_params[:handling_fee_rate],
    ...
    request_out_params.each do |record|
      ActiveRecord::Base.transaction do
        ...
        StockInout.create(
          ...
          warehouse_category_id:stock_out_params[:warehouse_category_id],
          user_id:              current_user.id
        )
  ...
  def request_stock_in_params
    params.require(:stock_inout).map do |uparams|
      {
        stock_inout_id:       uparams["stock_inout_id"],
        ...
        warehouse_category_id:uparams["warehouse_category_id"]
