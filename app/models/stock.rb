...
class Stock < ApplicationRecord
  ...
  scope :prepare_bill_amounts, -> (from_date = nil, to_date = nil, shipper_id = nil, warehouse_id = nil, page = nil, limit = nil) {
    query = <<-SQL
     SELECT  stock_id, current_bill_amounts.*, latest_bill_amounts.current_stock_amount as previous_stock_amount, 
        (COALESCE(latest_bill_amounts.current_stock_amount, 0)+COALESCE(current_bill_amounts.first_half_instock_amount, 0)+COALESCE(current_bill_amounts.mid_instock_amount,0)+COALESCE(current_bill_amounts.second_half_instock_amount,0)-
         COALESCE(current_bill_amounts.first_half_outstock_amount,0)-COALESCE(current_bill_amounts.mid_outstock_amount,0)-COALESCE(current_bill_amounts.second_half_outstock_amount)) as current_stock_amount,
         (COALESCE(latest_bill_amounts.current_stock_amount, 0)+COALESCE(current_bill_amounts.mid_instock_amount,0)+COALESCE(current_bill_amounts.second_half_instock_amount)-
          COALESCE(current_bill_amounts.first_half_outstock_amount,0)-COALESCE(current_bill_amounts.mid_outstock_amount,0)-COALESCE(current_bill_amounts.second_half_outstock_amount)) as total_inout_stock_amount
        FROM (
        ...
        WHERE inout_on BETWEEN '#{from_date}' AND '#{to_date}' AND is_billed = 0
  ...
  scope :prepare_bill_amounts_20, -> (from_date = nil, to_date = nil, shipper_id = nil, warehouse_id = nil, page = nil, limit = nil) {
    query = <<-SQL
      SELECT  current_bill_amounts.*, latest_bill_amounts.current_stock_amount as previous_stock_amount
      FROM (
        SELECT stock_id, MIN(lot_number) as lot_number, products.name as product_name, products.id as product_id,
        ...
        JOIN stocks on stocks.id = stock_inouts.stock_id
        JOIN products on products.id = stocks.product_id
        WHERE inout_on BETWEEN '2024-03-01' AND '2024-03-21' AND is_billed = 0
        GROUP BY stock_id, lot_number
      ) current_bill_amounts
      LEFT JOIN (
        SELECT bill_amounts.*
  ...
  scope :get_uncalc_bills ,-> (from_date, to_date, shipper_id, warehouse_id, page = nil, limit = nil) {
    sql = <<-SQL
    SELECT (in_stock_amount*DATEDIFF('#{to_date}', in_stock_date)-out_stock_amount*DATEDIFF('#{to_date}',out_stock_date)) as storage_cost, handle_cost, tt.shipper_id, received_payment_amount,
	previous_bill_amount, previous_bill_amount-received_payment_amount+handle_cost+(out_stock_amount*DATEDIFF('#{to_date}',out_stock_date))-(in_stock_amount*DATEDIFF('#{to_date}', in_stock_date)) as current_bill_amount,
   shippers.name as shipper_name, shippers.code as shipper_code
      ...
      FROM (
        ...
        FROM stock_inouts
        JOIN stocks ON stocks.id = stock_inouts.stock_id
        WHERE inout_on BETWEEN '#{from_date}' AND '#{to_date}' AND shipper_id='#{shipper_id}' AND warehouse_id='#{warehouse_id}' AND is_billed=0
      ) t
      GROUP BY shipper_id
      ) tt
      LEFT JOIN (
  ...
  scope :get_uncalc_bill_by_shipper ,-> (from_date, to_date, shipper_id) {
    sql = <<-SQL
    SELECT (in_stock_amount*DATEDIFF('#{to_date}', in_stock_date)-out_stock_amount*DATEDIFF('#{to_date}',out_stock_date)) as storage_cost, handle_cost, tt.shipper_id, received_payment_amount,
	previous_bill_amount, previous_bill_amount-received_payment_amount+handle_cost+(out_stock_amount*DATEDIFF('#{to_date}',out_stock_date))-(in_stock_amount*DATEDIFF('#{to_date}', in_stock_date)) as current_bill_amount,
   shippers.name as shipper_name, shippers.code as shipper_code
      ...
      FROM (
        ...
        FROM stock_inouts
        JOIN stocks ON stocks.id = stock_inouts.stock_id
        WHERE inout_on BETWEEN '#{from_date}' AND '#{to_date}' AND shipper_id='#{shipper_id}'
      ) t
      GROUP BY shipper_id
      ) tt
      LEFT JOIN (
